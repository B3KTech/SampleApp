...
import com.atakmap.android.pulsetool.plugin.ui.sync.LoggingFragment;
...
public class ZeroCrossingLoggingChart extends GHLineChart
{
    ...
    {
        if(result != null && result.getZcLog() != null)
        {
            updateDataSet(new ChartData(result.getZcLog().getZeroCrossingCount(), updateTime), getContext().getString(R.string.zc_logging_dataset));
            updateDataSet(new ChartData(Long.valueOf(result.getZcLog().getEnergyTotal()).intValue(), updateTime), getContext().getString(R.string.energy_logging_dataset));
            updateGHChart(new ChartData(result.getZcLog().getZeroCrossingCount(), updateTime));
        }
    }
